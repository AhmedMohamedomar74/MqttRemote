...
// Update this function to return the JSON command as a String
String IRModule::getJsonCommand() {
    if (jsonCommand == nullptr) {
        return String(); // Return an empty string if no JSON command is available
    }

    String jsonString;
    serializeJson(*jsonCommand, jsonString); // Serialize JSON to a String
    return jsonString;
}
...
void IRModule::resetData() {
    ...
    if (rawDataList) {
        Serial.println("Deleting rawDataList...");
        delete[] rawDataList;
        rawDataList = nullptr;
    }
    signalCount = 0;
    ...
    if (jsonCommand) {
        Serial.println("Deleting jsonCommand...");
        delete jsonCommand;
        jsonCommand = nullptr;
    }
    ...
    switch (protocol) {
        ...
        case MIDEA:
            for (size_t i = 0; i < rawDataLength; i++) {
                ...
                switch (protocol) {
                    case SAMSUNG: 
                      irsend->sendSAMSUNG(rawDataArray[i], 32);
                      Serial.println("Send data for samsung"); 
                      // Serial.println((rawDataArray[i].as<uint32_t>(), HEX);
                      break;
